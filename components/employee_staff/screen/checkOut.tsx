...
import { geyBookingUtilityByBookingId } from "@/service/BookingUtilityAPI"; // 👈 ĐÃ BỎ COMMENT
...
// ❌ Bỏ đối tượng tranFormData template không sử dụng
// const tranFormData = { ... };
...
export default function Checkout() {
  ...
  const [costDetailsForModal, setCostDetailsForModal] = useState(null); // 👈 State chính cho modal
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState(null);

  // ✨ State riêng để lưu services và damagedItems
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // ✨ SỬA: Thêm getServicesByBookingId vào Promise.all
        const [bookingDetails, historyDetails, payments, listEmployee, servicesData] = await Promise.all([
          getBookingById(bookingId),
          getHistoryBookingsByBookingId(bookingId),
          getPaymentsByBookingId(bookingId),
          getEmployeeByHotel(hotelId),
          geyBookingUtilityByBookingId(bookingId) // 👈 GỌI API DỊCH VỤ
        ]);
        ...
        if (checkInRecord && bookingDetails.room?.price) {
          ...
          // ✨ SỬA: Xây dựng dữ liệu cho modal
          const modalData = {
            roomDetails: {
            ...
            services: servicesData || [], // 👈 Thêm services từ API
            damagedItems: [], // 👈 Thêm mảng rỗng (sẽ được cập nhật sau)
            bookingId: bookingId, 
            isPaid: isPaid, 
          };
          setCostDetailsForModal(modalData); // 👈 Set state
        }
  ...
  // ✨ MỚI: Hàm callback nhận dữ liệu từ StaffListModal
  const handleReportReceived = (receivedDamagedItems) => {
    console.log("Nhận được báo cáo hư hỏng từ StaffListModal:", receivedDamagedItems);
    
    // 1. Cập nhật state damagedItems (nếu cần)
    setDamagedItems(receivedDamagedItems || []);

    // 2. Cập nhật state costDetailsForModal để thêm damagedItems vào
    setCostDetailsForModal(prevCostDetails => ({
      ...prevCostDetails,
      damagedItems: receivedDamagedItems || [] // Cập nhật mục vật dụng hỏng
    }));
    
    // 3. Cập nhật tiến trình
    setCurrentStep(3);
    
    // 4. Đóng modal nhân viên
    setStaffModalVisible(false);
    
    // 5. (Tùy chọn) Tự động mở modal chi phí để xem
    // setCostModalVisible(true);
  };

  // (Giao diện loading và error giữ nguyên)
  if (isLoading) {
    return (
  ...
  return (
    <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false}>
        {/* (Phần Header và Card giữ nguyên) */}
      ...
      <StaffListModal
        ...
        onReportReceived={handleReportReceived} // 👈 THÊM PROP CALLBACK
      />

      <CostDetailModal
        visible={costModalVisible}
        onClose={() => setCostModalVisible(false)}
        costData={costDetailsForModal} // 👈 TRUYỀN DỮ LIỆU ĐẦY ĐỦ
      />
    </View>
  );
}
